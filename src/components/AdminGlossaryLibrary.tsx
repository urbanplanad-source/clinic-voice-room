...
import { BookOpenCheck, ChevronDown, Download, Eye, FileUp, Loader2, PencilLine, Plus, Save, Search, Sparkles, Volume2, X } from "lucide-react";
...
type Entry = { id: string; scope: Scope; specialty?: HospitalSpecialty | null; hospitalId?: string | null; hospital?: { id: string; name: string; slug: string } | null; entryType: EntryType; spokenForms: string[]; standardKo: string; translations: Record<string, string>; category?: string | null; note?: string | null; priority: number; isActive: boolean };
...
export function AdminGlossaryLibrary() {
  ...
  const stats = useMemo(() => ({ total: entries.length, active: entries.filter((entry) => entry.isActive).length, hints: entries.filter((entry) => entry.entryType === "transcription_hint").length, verified: entries.filter((entry) => entry.entryType === "verified_sentence").length }), [entries]);
  ...
  async function toggle(entry: Entry) {
    setBusy(true);
    const response = await fetch(`/api/admin/glossary/${entry.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...payload(fromEntry(entry)), isActive: !entry.isActive }) });
    setBusy(false); if (!response.ok) { setError("활성 상태를 변경하지 못했습니다."); return; } await loadEntries();
  }
  ...
  return <div className={`${styles.root} space-y-5 pb-10`}>
    ...
    <section className="rounded-[24px] border border-line bg-white px-5 py-6 shadow-soft sm:px-7">
      <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
        <div><p className="flex items-center gap-2 text-sm font-semibold text-mint"><Sparkles className="h-4 w-4" /> 지속 학습 품질 자산</p><h1 className="mt-2 text-2xl font-bold tracking-[-0.03em] sm:text-3xl">품질 자산 라이브러리</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">검수에서 승인한 표현을 STT, 의료 용어, 검증 문장으로 관리합니다. 활성 자산은 다음 음성 인식과 번역에 재사용됩니다.</p></div>
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="전체 자산" value={stats.total} icon={<BookOpenCheck className="h-5 w-5 text-trust" />} /><Metric label="활성 자산" value={stats.active} icon={<Sparkles className="h-5 w-5 text-mint" />} /><Metric label="STT 힌트" value={stats.hints} icon={<Volume2 className="h-5 w-5 text-violet-500" />} /><Metric label="검증 문장" value={stats.verified} icon={<BookOpenCheck className="h-5 w-5 text-amber-500" />} /></div>
    </section>
    ...
    <section className="overflow-hidden rounded-[24px] border border-line bg-white shadow-soft">
      ...
      <div className="hidden grid-cols-[minmax(240px,1.7fr)_140px_160px_110px_110px_92px] gap-4 border-b border-line bg-slate-50 px-7 py-3 text-xs font-bold text-slate-400 lg:grid"><span>품질 자산</span><span>유형</span><span>적용 범위</span><span>번역 커버리지</span><span>상태</span><span className="text-right">관리</span></div>
      {loading ? <div className="flex min-h-48 items-center justify-center text-sm text-slate-500"><Loader2 className="mr-2 h-5 w-5 animate-spin" /> 불러오는 중</div> : entries.length === 0 ? <div className="px-6 py-16 text-center"><BookOpenCheck className="mx-auto h-9 w-9 text-slate-300" /><p className="mt-3 font-semibold">조건에 맞는 품질 자산이 없습니다.</p><p className="mt-1 text-sm text-slate-500">검수 대기함에서 승인하거나 새 자산을 추가해 보세요.</p></div> : <div className="divide-y divide-line">{entries.map((entry) => <article key={entry.id} className="grid gap-4 px-5 py-5 transition hover:bg-slate-50/70 sm:px-7 lg:grid-cols-[minmax(240px,1.7fr)_140px_160px_110px_110px_92px] lg:items-center"><div className="min-w-0"><p className="truncate font-bold tracking-[-0.01em]">{entry.standardKo}</p><p className="mt-1 truncate text-xs text-slate-500">{entry.spokenForms.length ? `발음 변형: ${entry.spokenForms.join(" · ")}` : "발음 변형 없음"}{entry.category ? ` · ${entry.category}` : ""}</p></div><div><Badge tone={entry.entryType === "transcription_hint" ? "violet" : entry.entryType === "verified_sentence" ? "amber" : "blue"}>{typeLabels[entry.entryType]}</Badge></div><div className="text-sm font-semibold text-slate-600">{entry.scope === "hospital" ? entry.hospital?.name || scopeLabels[entry.scope] : entry.scope === "specialty" && entry.specialty ? hospitalSpecialtyLabels[entry.specialty] : scopeLabels[entry.scope]}</div><div className="text-sm"><b>{coverage(entry)}</b><span className="text-slate-400"> / 17개</span></div><button type="button" disabled={busy} onClick={() => void toggle(entry)} className={`w-fit rounded-full px-3 py-1.5 text-xs font-bold ${entry.isActive ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>{entry.isActive ? "활성" : "비활성"}</button><button type="button" onClick={() => startEdit(entry)} className="inline-flex min-h-10 items-center justify-center gap-1.5 rounded-xl border border-line px-3 text-sm font-semibold hover:bg-white"><PencilLine className="h-4 w-4" /> 수정</button></article>)}</div>}
    </section>
