...
export function AdminGlossaryLibrary() {
  ...
  async function lifecycleAction(entry: Entry, action: "approve" | "activate" | "new_version" | "retire" | "rollback") {
    setBusy(true); setError(""); setNotice("");
  ...
  return <div className={`${styles.root} space-y-5 pb-10`}>
    ...
    <section className="rounded-[24px] border border-line bg-white px-5 py-6 shadow-soft sm:px-7">
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="전체 버전" value={stats.total} icon={<BookOpenCheck className="h-5 w-5 text-trust" />} /><Metric label="배포 중" value={stats.active} icon={<Rocket className="h-5 w-5 text-mint" />} /><Metric label="승인 대기 초안" value={stats.drafts} icon={<GitBranch className="h-5 w-5 text-violet-500" />} /><Metric label="검증 문장" value={stats.verified} icon={<BookOpenCheck className="h-5 w-5 text-amber-500" />} /></div>
    </section>
...
function GlossaryEntryRow({ entry, busy, onEdit, onLifecycle }: {
  ...
  onLifecycle: (entry: Entry, action: "approve" | "activate" | "new_version" | "retire" | "rollback") => Promise<void>;
}) {
  ...
  return <article className="grid gap-4 px-5 py-5 transition hover:bg-slate-50/70 sm:px-7 lg:grid-cols-[minmax(260px,1.5fr)_120px_150px_100px_minmax(260px,1fr)] lg:items-center">
    ...
    <div className="space-y-1"><Badge tone={tone}>{lifecycleLabels[entry.lifecycle]}</Badge><p className="text-xs font-bold text-slate-400">v{entry.version}</p></div>
    <div className="flex flex-wrap gap-2">
      {entry.lifecycle === "draft" ? <><button type="button" disabled={busy} onClick={() => onEdit(entry)} className="secondary-button min-h-10 px-3"><PencilLine className="h-4 w-4" /> 수정</button><button type="button" disabled={busy} onClick={() => void onLifecycle(entry, "approve")} className="secondary-button min-h-10 px-3"><ShieldCheck className="h-4 w-4" /> 승인</button></> : null}
      {entry.lifecycle === "approved" ? <button type="button" disabled={busy} onClick={() => void onLifecycle(entry, "activate")} className="primary-button min-h-10 px-3"><Rocket className="h-4 w-4" /> 배포</button> : null}
      {entry.lifecycle === "active" ? <><button type="button" disabled={busy} onClick={() => void onLifecycle(entry, "new_version")} className="secondary-button min-h-10 px-3"><GitBranch className="h-4 w-4" /> 새 버전</button><button type="button" disabled={busy} onClick={() => void onLifecycle(entry, "retire")} className="secondary-button min-h-10 px-3"><Archive className="h-4 w-4" /> 중지</button></> : null}
      {entry.lifecycle === "retired" ? <button type="button" disabled={busy} onClick={() => void onLifecycle(entry, "rollback")} className="secondary-button min-h-10 px-3"><RotateCcw className="h-4 w-4" /> 이 버전으로 롤백</button> : null}
    </div>
  </article>;
}
