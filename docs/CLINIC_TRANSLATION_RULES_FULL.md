# Clinic Translation Glossary And Normalization Rules

Source: `src/lib/clinic-glossary.ts`
Generated: 2026-06-19

이 문서는 병원 상담 번역에 현재 적용되는 용어집/정규화 규칙의 전문입니다.

검토 포인트:
- `brandDisplayByLanguage`: 리쥬란, 쥬베룩 등 브랜드 표기 방식
- `rawClinicGlossary`: 시술명/장비명/단위/증상 용어집 원문
- `criticalShortPhrases`: 짧은 상담/시술 문장 고정 번역
- `applyHighPriorityClinicCorrections`: 리쥬란 색상 박스, 부종, 오인식 보정
- `targetMisrecognitionCorrections`: 언어별 오역/오인식 후처리
- `normalizeClinicTranslation`: 번역 결과를 용어집 기준으로 다시 정리하는 순서
- `buildClinicGlossaryInstructions`: 모델에게 전달되는 용어집 지침

수정 의견을 주실 때는 원문을 직접 고쳐도 되고, `수정 전 -> 수정 후` 형태로 적어주셔도 됩니다.
